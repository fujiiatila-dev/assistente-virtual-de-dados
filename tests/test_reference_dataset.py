...
class ReferenceLLM:
    """Deterministic model double that still exercises the complete graph."""
...
def reference_db() -> Path:
    ...
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    pytest.skip(
        "Anexo oficial indisponível. Defina DB_PATH ou coloque anexo_desafio_1.db um nível acima."
    )
...
def _run(
    ...
    return DataAssistant(reference_db, llm=ReferenceLLM(sql, visualization)).ask(question)
